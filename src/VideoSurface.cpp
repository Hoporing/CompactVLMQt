    ...
    : QQuickPaintedItem(parent)
{
    ...
    m_timer->setInterval(16);  // ~60fps polling — eliminates beat frequency with Qt Quick render loop
...
bool VideoSurface::open(const QString &url, int w, int h)
{
    ...
    bool ok = m_player->OpenVideo(url, w, h);
    if (!ok) {
        m_player.reset();
        setStatusText("열기 실패: " + url);
        return false;
    }
...
void VideoSurface::onTimerTick()
{
    ...
    // Process only when a new frame has arrived (compare sequence number)
    ...
    // Use VLC delivery FPS (from CheckFPS) — most accurate
...
void VideoSurface::paint(QPainter *painter)
{
    ...
    // Letterbox rendering — preserves aspect ratio
